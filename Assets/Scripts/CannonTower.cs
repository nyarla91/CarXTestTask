...
public class CannonTower : Tower<CannonProjectile>
{
	...
	private void FixedUpdate()
	{
		...
		Vector3 forwardToTarget = (Target.PredictPosition(_predictTime) - Transform.position).WithY(0).normalized;
	...
	private void CalculatePredictionTime()
	{
		...
		for (int i = 0; i < _predictIterations; i++)
		{
			...
			_predictTime = CalculateFallTime(verticalSpeed, gravity, height);
			targetPosition = Target.PredictPosition(_predictTime);
			print($"iteration: {i} — {_predictTime}");
		}
	}
